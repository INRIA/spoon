 ...
 * LICENSE-MIT.txt) of the Cecill-C License (see LICENSE-CECILL-C.txt). You as
 * the user are entitled to choose the terms under which to adopt Spoon.
 */
package spoon;
...
public class IncrementalLauncher extends Launcher {
  ...
  private static CacheInfo loadCacheInfo(File file)
      throws InvalidClassException {
    try (FileInputStream fileStream = new FileInputStream(file);
         ObjectInputStream objectStream =
             new ObjectInputStream(new BufferedInputStream(fileStream))) {
  ...
  private static void saveCacheInfo(CacheInfo cacheInfo, File file) {
    try (FileOutputStream fileStream = new FileOutputStream(file);
         ObjectOutputStream objectStream =
             new ObjectOutputStream(new BufferedOutputStream(fileStream))) {
  ...
  private static void saveFactory(Factory factory, File file) {
    try {
      new SerializationModelStreamer().save(factory,
    ...
    for (File e : resources) {
      if (e.isDirectory()) {
        Collection<File> files = FileUtils.listFiles(
            e, new SuffixFileFilter(".java"), TrueFileFilter.INSTANCE);
        files.forEach(f -> {
          try {
            javaFiles.add(f.getCanonicalFile());
          } catch (IOException e1) {
            throw new SpoonException("unable to locate input source file: " +
                                     f);
          }
        });
   ...
   * @param cacheDirectory The directory to store all incremental information.
   *     If it's empty, full rebuild will be performed.
   * @param forceRebuild Force to perform full rebuild, ignoring incremental
   *     cache.
   * @throws IllegalArgumentException
   * @throws SpoonException
   */
  public IncrementalLauncher(Set<File> inputResources,
                             Set<String> sourceClasspath, File cacheDirectory,
    ...
    if (!mIncrementalCacheDirectory.exists() &&
        !mIncrementalCacheDirectory.mkdirs()) {
    ...
    if (forceRebuild) {
      ...
      mRemovedSources = new HashSet<>(CollectionUtils.subtract(
          mCacheInfo.inputSourcesMap.keySet(), mInputSources));
      mAddedSources = new HashSet<>(CollectionUtils.subtract(
          mInputSources, mCacheInfo.inputSourcesMap.keySet()));
      mCommonSources = new HashSet<>(CollectionUtils.intersection(
          mCacheInfo.inputSourcesMap.keySet(), mInputSources));
      ...
      try {
        ...
        throw new SpoonException("unable to locate class files dir: " +
                                 mClassFilesDir);
      }

      Collection<CtPackage> oldPackages = oldFactory.Package().getAll();
      for (CtPackage pkg : oldPackages) {
        if (pkg.getTypes().isEmpty() && pkg.getPackages().isEmpty() &&
      ...
      mChangesPresent = !mRemovedSources.isEmpty() ||
                        !mAddedSources.isEmpty() ||
                        !incrementalSources.isEmpty();
      setBinaryOutputDirectory(mClassFilesDir);
    }

    getEnvironment().setSourceClasspath(
        mSourceClasspath.toArray(new String[0]));
  }
  ...
  /**
   * Returns true, if any source code changes after previous build are present,
   * and false otherwise.
   */
  public boolean changesPresent() { return mChangesPresent; }

  /**
   * Caches current spoon model and binary files. Should be called only after
   * model is built.
   */
  public void saveCache() {
    ...
    for (Entry<String, CompilationUnit> e :
         factory.CompilationUnit().getMap().entrySet()) {
    ...
    if (mCacheInfo != null) {
      ...
      for (File r : mRemovedSources) {
        newSourcesMap.get(r).forEach(
            File::delete); // Removes corresponding .class files
    ...
    Collection<File> dirs = FileUtils.listFilesAndDirs(
        mClassFilesDir, DirectoryFileFilter.INSTANCE, TrueFileFilter.INSTANCE);
    dirs.stream()
        .filter(d
                -> d.exists() && FileUtils
                                     .listFiles(d, TrueFileFilter.INSTANCE,
