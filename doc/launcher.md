...
:warning: The `JarLauncher` feature (and all features relying on decompilation) are not included in `spoon-core` but in `spoon-bytecode`. If you wnat to use them you should declare a dependency to `spoon-bytecode`.
