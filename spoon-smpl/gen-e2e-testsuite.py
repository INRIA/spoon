...
def gen_suite(suitename, dirname, recursive=True):
    ...
    output.append("import spoon.reflect.CtModel;")
    output.append("import spoon.reflect.declaration.CtClass;")
    output.append("import spoon.support.compiler.VirtualFile;")
    output.append("import static spoon.smpl.TestUtils.*;")
    output.append("")
    
    output.append("public class {} {{".format(suitename))
    
    output.append("    private CtClass<?> getTargetClass(String code) {")
    output.append("        CtModel model = SpoonJavaParser.parse(code);")
    output.append("        return (CtClass<?>) model.getRootPackage()")
    output.append("                                 .getTypes()")
    output.append("                                 .stream()")
    output.append("                                 .filter(ctType -> ctType.getComments()")
    output.append("                                                         .stream()")
    output.append("                                                         .noneMatch(comment -> comment.getContent()")
    output.append("                                                                                      .contains(\"skip\")))")
    output.append("                                 .findFirst().get();")
    output.append("    }")
    output.append("")
    ...
    output.append("        CtClass<?> input = getTargetClass(inputCode);")
    output.append("        CtClass<?> expected = getTargetClass(expectedCode);")
    output.append("")
